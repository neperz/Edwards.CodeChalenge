...
RUN dotnet build "./src/Edwards.CodeChalenge.API/Edwards.CodeChalenge.API.csproj" -c Debug
CMD dotnet run --project src/Edwards.CodeChalenge.API/Edwards.CodeChalenge.API.csproj
...
COPY . .
RUN dotnet restore . -s $nuget_endpoint -s "https://api.nuget.org/v3/index.json"
RUN dotnet build "./src/Edwards.CodeChalenge.API/Edwards.CodeChalenge.API.csproj" -c Release

FROM build AS publish
RUN dotnet publish "./src/Edwards.CodeChalenge.API/Edwards.CodeChalenge.API.csproj" -c Release -o /publish

FROM mcr.microsoft.com/dotnet/aspnet:6.0 AS production
ENV ASPNETCORE_ENVIRONMENT=Production
WORKDIR /app
COPY --from=publish /publish .
ENTRYPOINT ["dotnet", "Edwards.CodeChalenge.API.dll"]
EXPOSE 80
